 ...
 *  - 上一帧跟踪：驱动内部维护 48KB 影子缓冲（优先 PSRAM），刷新时 DTM1 写入
 *    *真实*的旧图像，每个像素按 {旧,新} 组合选择正确的 LUT 波形——因此局部
 *    刷新可作用于任意背景（黑底画白、白底画黑均可）。清屏（image_buf=NULL）
 *    时 DTM1 强制 0x00，使全屏/窗口经历 黑->白 翻转，保证被真正驱动为纯白；
 *  - 局部刷新：CDI(0xA9,0x07) -> PTIN(0x91) -> PTL(0x90, 窗口) -> DTM1(0x10)
 *    -> DTM2(0x13) -> DRF(0x12) -> 等待 BUSY -> PTOUT(0x92)。x 与 width 必须
 *    为 8 的倍数（1bpp 按字节寻址）；
